...
1. AmAdaptor 模組: `XX`-config.json: `XX` 為兩個十六進位值 (`01`~`FF`，須大寫字母)
    ...
    * `devices`: 此為 MQ 系統組態檔所需資訊，所有的 `deviceID|funcID` 必須出現在 `map` 中 `key: value` 的 `value` 中。
    * `pulses`: 可定義某一銨茂模組的自動延時關閉 (ms)。如下例開門後 5 秒自動關閉門鎖，`devices` 下的 "`A02|DO09`" 功能 `value` 為 "`2`" (按下/放開: press/release)，故需定義 `pulses`，指定 5000 ms 後自動送出關閉命令
        ````json
        {
            "devList": {
                "02": "210"
    ...
    * `hex`: 敘述送出十六進位命令、群組 (狀態) 及 IR 等結構:
        ...
        + `port`: 僅 IR 模組用，表示要輸出到哪些輸出埠。共有 8 埠，以 bit 0~7 表示。
        + `defaultLatency`: 僅 IR 模組用，預設每一功能發射後多少毫秒再送出停止碼。
        + `latency`: 僅 IR 模組用，指定那些功能 ID 發射後多少毫秒再送出停止碼。\
        格式為: [ 毫秒, 功能ID1, 功能ID2, ... ]
        + `group`: 由於 Hex 或 IR 模組並無回饋狀態，但可利用 `group` 來指定哪些功能為一群組，每一群組中只允許其中一個狀態為 On，其餘為 Off。當設定群組中的某一功能為 On 時，其他的功能自動變更為 Off。
        + `codes`: 每一功能對應送出的 Hex 碼，如下所述:
