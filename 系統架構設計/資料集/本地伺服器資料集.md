...
Sn = [
    {   // 移轉物件
        "expression": "\"模組ID|設備ID|功能ID\":300 > 27",  // 温度超過27度持續5分鐘
        "actions": [ action, ... ],
        "next": 0                           // 0: 不移轉, 1~n: 移轉到 S1~Sn
    },
    // ...
]
...
* `transition`: 檢查條件 `expression` 是否成立，若是則執行 `actions` 命令後，再依 `next` 移轉到新的狀態，若 `next` 為 `null`，則繼續下一個移轉物件檢查。
* `expr` 中的 `duration` 表示該運算式需持續滿足多少秒 (精確度 0.1 秒) 才算成立。
