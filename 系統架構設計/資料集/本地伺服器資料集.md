...
1. 以下 `actions`、`scenes`、`states` 可用下列 BNF 語法表示之:
    ...
    ```bnf
    <action list> ::= "[" [ <concurrent> ] { <action>   | <action list> } "]"
    <scene list>  ::= "[" [ <concurrent> ] { <scene id> | <scene list>  } "]"
    <state list>  ::= "[" [ <concurrent> ] { <state id> | <state list>  } "]"
    <concurrent>  ::= '"' "C" '"' [ "," ]
...
1. 情境:
    ...
    * `scenes.mode` 同一情境重疊執行時，以 bit1 (舊情境)、 bit0 (新情境) 表示是否執行之。也就是 bit1 為零時取消舊的情境 (如果有的話)，bit0 為1時執行新的情境。
    <br>
...
1. 關注: 只接收控制 (推播 iOS, Android 訊息)，不需狀態回報
    ...
    * 要有 Web API 使 Apps 能獲取到最近 (三個月) 的推播訊息。
