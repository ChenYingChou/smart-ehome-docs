...
expr = {    // 運算後返回值 -- bit0: 0:不符/1:符合, bit1: 滿足持續時間
    "op": op_binary || op_unitary,
    "left": expr_var,   // 變數 $1~$n, 或運算式
    "right": expr_var,  // 變數 $1~$n, 或運算式
    "duration": 0,      // 要求持續秒數 (精確度 0.1 秒)
    "start_at": 0       // 開始計算時間 (ms)
}
...
* `transition`: 檢查條件 `condition` 是否成立，若是則執行 `actions` 命令後，再依 `state` 移轉到新的狀態，若 `state` 為 `null`，則繼續下一個移轉物件檢查。
