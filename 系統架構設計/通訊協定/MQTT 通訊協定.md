...
1. 伺服器回應新版本的模組/設備及其下的功能組態:
    ...
    * `functions` 的 `value`: 定義功能輸出(控制)或讀取(回應)的值域
        ...
        |   值  |    說明                         | 控制狀態值 | 回應狀態值
        |:-----:|--------------------------------|----------|---
        |   1   | 一次觸發 (pulse)                | 1 或自定  | 0:關, 1:開, 2:警示
    ...
    * 狀態 `Si`{i=1\~n} 為一群移轉物件陣列 `T[]`:
        ...
        * `expression`: 可為各項 [`模組|設備|功能`]`:持續秒數` 值的條件比較運算式，`:持續秒數` 為可選項，例如:\
        ...
        運算子 (符號) 及優先順序
            ...
            1. 運算式中的外部變數須用 `雙引號`、`單引號` 或 `方括號` 包起來，外部變數是指本系統中 `模組｜設備｜功能` 的狀態值。當外部變數異動時，會引發現行狀態對應的移轉物件 (有引用此外部變數者) 進行評估處理。
            1. 智慧控制內部變數: (不分大小寫，於運算式中使用，不須用引號或方括號包起來)

                內部變數 | 說明
                :------:|---
                `errors` | 現行狀態 `Si` 執行週期時所發生的錯誤數<br>此錯誤是指 `T[1~m].error` 引發的次數
        ...
        * `actions`: 當 `expression` 評估滿足 (`true`) 時要執行的命令，請參照前項情境 (SCENES) 說明。
            ...
            1. 在本智慧控制結束時，亦可指定執行 `actions`，此時會等待 0.25 秒就強迫結束相關的 `actions` 執行 (包含此智慧控制累計尚待執行的命令)。
            1. 智慧控制的結束有兩種成式:
                * `next` 設為 `-1`，見下面 `next` 欄位說明。
                * 由 App 或排程觸發停用本智慧情境，此時在 `expression` 可用 `self` 來偵測是否被中止，若是則可執行一些收尾命令。如後面的[保全智慧控制範例](#security)，在結束時推播 "保全關閉" 語音及訊息，並關閉警鈴。為何要關閉警鈴? 因為可能保全執行中符合指定條件而觸發警鈴，但設定為 N 秒後才關閉之，可是因為本智慧控制在這 N 秒到達前被停用，停用時會將之前尚待執行的命令取消掉，因此警鈴會響不停，所以才要在收尾時多執行一個關閉警鈴的命令。
        ...
        * `next`: 當 `expression` 評估滿足 (`true`) 時是否要移轉到其他狀態。
            1. `0` 表示繼續下個移轉物件處理。
            1. `1~n` 表示移轉到 `S1~Sn`，結束目前狀態，以下的移轉物件也不再進行評估處理。
            1. `-1` 表示結束此狀態機，相當於停用此智慧控制。
        * `interval`: 當 `expression` 評估滿足 (`true`) 且未移轉到其他狀態 (`next` = 0) 時，本欄位值若不為零，則表示多少秒之後才會再處理本移轉物件處理，即使在此計時前有變數異動也不理會。\
...
* 當指定 `伺服器ID|模組ID|設備ID|功能ID` 狀態有變更時，系統會自動將該訊息轉到 MQTT 主題 `attn/<mid>`，格式同 [模組/設備/功能狀態變更 (cmd=2)](#模組設備功能狀態變更-cmd2)，只是多了時間戳記:
    ```js
    {
        "cmd": 2,
