...
1. 伺服器回應新版本的模組/設備及其下的功能組態:
    ...
    * `functions` 的 `value`: 定義功能讀取或輸出的值域
        ...
        |   3   | 按下/放開 (press/release)        | 1:按下, 0:放開
...
1. <b id="scenes">`SCENES`</b> 情境:
    ...
    * `active`: 同通訊模組狀態變更回報。參見 [模組/設備/功能狀態變更](#模組設備功能狀態變更)。對應代入: 模組=`$00`，設備=`SCENES`，功能=`情境ID`。情境在觸發控制時會回報狀態為 `1`，當所有動作執行完後會回報為 `0`，這段時間通常不會持續太久。
...
* 若系統模組資料有異動時，對應的版號會加一。此時查詢 [伺服器下各模組/設備組態](#查詢伺服器下各模組設備組態) 的版號/內容會有所不同。
