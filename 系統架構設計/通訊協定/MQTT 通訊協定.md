...
1. 伺服器回應新版本的模組/設備及其下的功能組態:
    ...
    * `functions` 的 `type`: 定義功能為可讀取(狀態)或可輸出(控制)
        ...
        <sup>[註] 若為警示之類的狀態，請回報 1 表示異常、0 表示正常狀態，以利接收端應用程式統一判斷。</sup>
    ...
    * `functions` 的 `value`: 定義功能讀取或輸出的值域
        ...
        |  100  | 調光 (0~100%), 線性時間 (單位秒) | 0~100[,線性時間]
...
1. 系統廣播訊息:
    ...
    * App 可自行決定如何處理本訊息，例如: 一般或錯誤訊息以彈出視窗顯示數秒後自行消失，而警告訊息則一直保持在畫面最上層，直到按下關閉鍵才消失。
