...
App → 本地伺服器 | `to/$YS`
App → 雲端伺服器 | `to/<sid>/$YS`
<sub>註: MQTT 主題 為 [`<WebAPI>/login`](./Web%20API.md#app-登錄取得取得-mqtt-通訊時所需的資訊) 時所回應的 `topic.pub` + `$YS`</sub>
...
1. App 請求伺服器所有模組組態:
    ...
    * `__loginid__` 為客端設備登入的帳號(loginid)。
...
1. 本地伺服器收到狀態變更:
    ...
    * 儲存到內部記錄: 經由 MQTT 轉發到 App 主題區，所有 App 將會收到如下:
