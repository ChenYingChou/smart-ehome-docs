...
1. 伺服器回應新版本的模組/設備及其下的功能組態:
    ...
    * `functions` 的 `value`: 定義功能輸出(控制)或讀取(回應)的值域
        ...
        |   值  |    說明                         | 控制狀態值 | 回應狀態值
        |:-----:|--------------------------------|----------|---
        |   1   | 一次觸發 (pulse)                | 1 或自定  | 0:關, 1:開, 2:警示<br>推播則為正進行中的筆數
        |   2   | 開/關 (on/off) 或<br>警報信號       | 0:關, 1:開,<br>2:切換 | 0:關/正常, 1:開/異常,<br>2:警示
        |   3   | 按下/放開 (press/release)       | 0:放開, 1:按下 | 0:放開, 1:按下, 2:警示
        |  100  | 調光 (0\~100%), 線性時間 | 0\~100[,線性秒數] | 0\~100
        | n1\~n2 | 指定某段範圍值, 如: 温度          | 指定範圍值 | 指定範圍值
        | 其他  | 模組自行定義 (不建議) | 自定 | 自定
        | | 設備/功能已失聯 | 無 | `null`

        <sup>[註] 回應狀態值 `2:警示` 表示在 `1:開啟/按下` 情況下的警示，例如保全智慧控制時發生入侵事件。</sup>
...
1. 本地伺服器收到狀態變更:
    ...
    * `_timestamp_` 為自 1970-01-01 (UTC) 起的毫秒數。對 App 而言，只要記住伺服器最後一次的時間戳記，在斷線重連時要求重送此時間後的更新狀態即可，見隨後 [查詢模組/設備/功能最新狀態 (cmd=4)](#查詢模組設備功能最新狀態-cmd4)。
...
* 若要同時控制多組設備/功能狀態時， `payload` 可用陣列表示，但其中每個項目的 `伺服器ID|模組ID` 必須相同，且 `模組ID` 必須等於 MQTT 主題中的 `<mid>`。例如:\
        ...
        * `expression` (條件運算式): 可為各項 [`模組|設備|功能`]`:持續秒數` 值的條件比較運算式，`:持續秒數` 為可選項，例如:\
        ...
        運算子 (符號) 及優先順序
            ...
            1. 智慧控制內部變數: (不分大小寫，於運算式中使用，不可用引號或方括號包起來)
                ...
                `self` | 指目前智慧控制本身狀態，0:被中止, 1:執行中, 2:執行中+警示<br>相當於外部變數 [`$00｜WISDOMS｜本智慧控制ID`]
...
* 若系統模組基本資料有異動時 (指 [`cmd=101`](#伺服器回應各模組設備組態-cmd101) 回應 `devices`、`functions` 的 `name`)，對應的版號會加一。此時查詢 [伺服器下各模組/設備組態 (cmd=1)](#查詢伺服器下各模組設備組態-cmd1) 的版號/內容會有所不同。
...
* 當指定 `伺服器ID|模組ID|設備ID|功能ID` 狀態有變更時，系統會自動將該訊息轉到 MQTT 主題 `attn/<mid>`，格式同 [模組/設備/功能狀態變更 (cmd=2)](#模組設備功能狀態變更-cmd2)，但多了時間戳記:
    ```js
    {
        "cmd": 2,
...
* 如果第三方應用程式已註冊 MQTT 主題 `from/#` 時，表示已能收到所有模組/設備/功能狀態的變化，那麼就沒必要再用此關注功能。此關注功能主要是給第三方應用程式，只能接收特定的模組/設備/功能變化。一般而言通訊模組並不接收其他模組的狀態變化，只接受伺服器管理的查詢及 App 控制該模組設備/功能的請求。
...
1. 通訊模組第一次要先向本地伺服器註冊，送出組態結構，並取得模組身份驗證令牌。
    ...
    * `devices` 結構內容請參考 [伺服器回應各模組/設備組態 (cmd=101)](#伺服器回應各模組設備組態-cmd101)。
...
1. 本地伺服器連線到雲端:
    ...
    1. 送出本地的各模組組態結構，參考 [伺服器回應各模組/設備組態 (cmd=101)](#伺服器回應各模組設備組態-cmd101)。
