...
`通訊模組 → 本地伺服器`、`本地伺服器 → 雲端伺服器`、`伺服器 → App`
...
1. 通訊模組第一次要先向本地伺服器註冊，送出組態結構，並取得模組身份驗證令牌。
    ...
    <sup>註: `devices` 結構內容請參考 [回應伺服器下各模組/設備組態](#回應伺服器下各模組設備組態)。<sup>
...
1. 心跳及回應:
    ...
    `雲端伺服器 → 伺服器本地`、`本地伺服器 → 通訊模組`、`伺服器 → App`
    ...
    * 收到心跳訊息時，回應並帶上自己的 ID 回覆之:
        ...
        + App 回應雲端伺服器: `"c_id": "__客戶端ID__"`。

1. 用戶 `token` 已失效，請重新登錄取得新的身份驗證令牌: 伺服器 → App
