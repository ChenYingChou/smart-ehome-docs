...
1. App 請求伺服器所有模組組態:
    ...
    * 若連接本地伺服器，`s_id` 可給予空字串，或用 `"*"` 表示此次連線所要求的伺服器。
    * `<version>` 為版本數值，若伺服器版本號碼大於請求方的值，則會送出該伺服器所有模組組態。現存的版本號號從 1 計起，每次組態有變更則加 1，因此請求方的 `<version>` 若為零則表示要求伺服器重送出所有模組組態。
...
1. 若 `__伺服器ID__`、`__模組ID__` 或 `__設備ID__` 已不存在，則返 `<version>` 為 `0`:
...
1. 各通訊模組在設備/功能狀態有變更時，主動向本地伺服器送出最新狀態，並加上 `token ` 模組身份驗證令牌:
...
1. 本地伺服器收到狀態變更:
    ...
    於 `payload` 欄位的 `伺服器ID` 換置成 `本地伺服器ID`，增加時戳欄位 `<timestamp>`，並將 `token` 欄位換成雲端系統給予的身份驗證令牌後，送往雲端系統:

    ```js
    {
        "cmd": 2,
        "payload": "伺服器ID|模組ID|設備ID|功能ID|最新狀態值",
        "timestamp": <timestamp>,
        "token": "本地伺服器身份驗證令牌"
    }
    ```

    * `<timestamp>` 為自 1970-01-01 (UTC) 起的毫秒數。對 App 而言，只要記住伺服器最後一次的時間戳記，在斷線重連時要求重送此時間後的更新狀態即可，見隨後 [查詢模組/設備/功能最新狀態](#查詢模組設備功能最新狀態)。
    * 儲存到內部記錄: 移除 `token` 欄位，經由 MQTT 轉發到 App 主題區，所有 App 將會收到如下:
...
1. App 啟動或斷線重連後要求各模組/設備/功能的最新狀態:
    ...
    {
        "cmd": 4,
        "payload": "伺服器ID|模組ID|設備ID|功能ID|<timestamp>"
    }
    ```

* 當本地伺服器重新連線時，雲端系統亦會向本地伺服器發出本查詢狀態請求。
* `<timestamp>` 表示查詢這個時間點後有異動的狀態，如果為零則表示所有的狀態。
* `payload` 中的 `模組ID|設備ID|功能ID` 可以由右向左省略，表示要求某一項目以下的狀態:
    * `伺服器ID|模組ID|設備ID|功能ID|<timestamp>` 查單一功能狀態。
    * `伺服器ID|模組ID|設備ID|<timestamp>` 查此設備下所有功能狀態。
    * `伺服器ID|模組ID|<timestamp>` 查此模組下所有設備/功能狀態。
    * `伺服器ID|<timestamp>` 查此伺服器下所有模組/設備/功能狀態。
...
如果自指定時間後都沒有異動，則返回 `modules` 為空物件:
...
1. 通訊模組第一次要先向本地伺服器註冊，送出組態結構，並取得模組身份驗證令牌。
    ...
    * `devices` 結構內容請參考 [回應伺服器下各模組/設備組態](#回應伺服器下各模組設備組態)。
    * 接收本地伺服器回應 JSON 格式:

    ```js
    {
        "cmd": 120,                         // 本地伺服器 → 通訊轉接模組
        "m_id": "**自定模組ID**",            // 註冊時送出的自定模組ID
        "status": 0,                        // 0:成功, 非零:錯誤
        "payload": "**msg**"                // 錯誤訊息
    }
    ...
    {
        "id": "__本地模組唯一ID__",         // 若模組ID有衝突, 則為新模組ID, 否則不變同原有送出的模組ID
        "token": "__身份驗證令牌__"         // 以後送往 Server 要帶上此值
    }
    ```
...
1. 本地伺服器連線到雲端:
    ...
    1. 身份驗證令牌連結到本地伺服器後，伺服器連線雲端主機註冊。
        ...
        * 接收本地伺服器回應 JSON 格式:
        ...
        // 註冊成功後解密 payload 為 JSON 字串如下:
        {
            "id": "__伺服器唯一ID__",            // 若模組ID有衝突, 則為新伺服器ID, 否則不變同原有送出的自定伺服器ID
            "token": "__伺服器身份驗證令牌__"     // 以後送往雲端系統要帶上此值
        }
        ```

        * 註冊失敗返回 payload 為錯誤訊息。
        * 若伺服器 ID 與已註冊伺服器有衝突時，則會返回 `__伺服器唯一ID__`。以後通訊則必須使用這個指定的伺服器 ID。

    1. 送出本地的各模組組態結構，參考 [回應伺服器下各模組/設備組態](#回應伺服器下各模組/設備組態)。
...
1. 心跳及回應: 通訊轉接模組 ← 本地伺服器 ← 雲端伺服器 → Apps
...
1. 用戶專案的備份資訊
    ...
    * App 每一訊息都要求帶 `token`，由 `token` 可識別出用戶 ID。
