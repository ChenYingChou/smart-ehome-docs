...
1. <b id="wisdoms">`WISDOMS`</b> 智慧控制:
    ...
    * `active` 為智慧控制的狀態值，`0` 表示停用，`1` 表示啟用。當使用 [`控制模組/設備/功能`](#控制模組設備功能) 將智慧控制由啟用改成停用時，將會取消目前正在執行中的智慧控制，即使智慧控制正執行了一半也會中止。例如: 智慧控制每小時執行一次，每次開燈半小時後關掉，若在未到半小時前取消智慧控制，這時半小時後要關燈的工作也會一併取消，這時燈會一直保持著開的狀態。
    * `states` 狀態機為一陣列，其內每個元素 `Si`{i=1~n} 為一群移轉物件陣列 `Ti`:
        ...
        * `expression`: 可為各項 "`模組|設備|功能`"[`:持續秒數`] 值的條件比較運算式，`:持續秒數` 為可選項，例如:\
        `"模組ID|設備ID|功能ID":300 >= 27`  // 某功模組設備能值(如温度) 持續 300 秒達到 27 (度) 幾上
            運算子 | 優先序 (高到低) | 說明
            :---:|:---:|---
            `!` `~` | 最高 | 一元運算(unary): 反邏輯 (true, false)、反位元 (位元 0 <-> 1)
            `*` `/` `%` | 左到右 | 數值: 乘、除、整除取餘數
            `+` `-` | 左到右 | 數值: 加、減
...
1. <b id="schedules">`SCHEDULES`</b> 排程:
    ...
    * `active` 為排程的狀態值，`0` 表示停用，`1` 表示啟用。當使用 [`控制模組/設備/功能`](#控制模組設備功能) 將排程由啟用改成停用時，將會取消目前正在執行中的排程，即使排程正執行了一半也會中止。例如: 排程每小時執行一次，每次開燈半小時後關掉，若在未到半小時前取消排程，這時半小時後要關燈的工作也會一併取消，這時燈會一直保持著開的狀態。
...
* 若系統模組基本資料有異動時 (指 [`cmd=101`](#伺服器回應各模組設備組態) 回應 `devices`、`functions` 的 `name`)，對應的版號會加一。此時查詢 [伺服器下各模組/設備組態](#查詢伺服器下各模組設備組態) 的版號/內容會有所不同。
...
* 當指定 `伺服器ID|模組ID|設備ID|功能ID` 狀態有變更時，系統會自動將該訊息轉到 MQTT 主題 `attn/<mid>`，格式同 [模組/設備/功能狀態變更](#模組設備功能狀態變更)，只是多了時間戳記:
    ```js
    {
        "cmd": 2,
