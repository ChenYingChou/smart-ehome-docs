...
```js
const mqServer = {   // 來自 Web API: UDP, 請選 "local" 或 "inet"
...
const mqtt_info = { // 來自 Web API: login, 本項在連接本地或雲端服務器時值會不同
    "user": "D2587",  // 此值在連雲端服務器時不同, 例如: "AM2F-D2587"
...
MQTT 主題 | `<to>` | Web API login 取得的 `topic.pub`。
...
<sup>註: MQTT 主題 為 [`<WebAPI>/login`](./Web%20API.md#app-登入取得-mqtt-通訊時所需的資訊-login) 時所回應的 `topic.pub` (即 `<to>`) + `"$YS/"` + `<cid>`</sup>
