...
1. App 送出 `POST` 資料如下:
    ...
    * `<設備名稱>`: 請 App 自行定義，如: iPhone、iPad、Sony XZ ...，以供後續調用區別。
    * `<授權碼>`: 此欄位在新設備註冊前需向系統管理者取得之。若未帶此授權碼，除了第一個註冊的設備外，其餘會回報錯誤如下所述。
...
1. 網頁伺服器回覆:
    ...
    * 若收到 `status` = `2`，請向系統管理者 (第一個註冊者) 取得新授權碼後再試一次。
    * 每次授權碼使用後會失效，因此系統管理者要為每個新設備取得授權碼。每次取得的授權碼有效期間只有 4 個小時，超過時間後無法註冊新設備，請再重新取得授權碼註冊設備。
