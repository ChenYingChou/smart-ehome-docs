...
本功能為 [MQTT 通訊協定 - 查詢伺服器下各模組設備組態](./MQTT%20通訊協定.md#查詢伺服器下各模組設備組態) 中原有功能，只是改用 Web API 取得伺服器下所有模組設備組態。
...
1. 網頁伺服器回覆:
    ...
    * 成功時 `payload` 內容詳見 [MQTT 通訊協定 - 伺服器回應各模組/設備組態](./MQTT%20通訊協定.md#伺服器回應各模組/設備組態)。


## App 取得取得 MQTT 系統中所有模組/設備/功能最新狀態

URI = [`<WebAPI>`](#網站連線主機名稱)`/mq_statusreq`

本功能為 [MQTT 通訊協定 - 查詢模組/設備/功能最新狀態](./MQTT%20通訊協定.md#查詢模組設備功能最新狀態) 中原有功能，只是改用 Web API 取得伺服器下所有模組/設備/功能最新狀態。
